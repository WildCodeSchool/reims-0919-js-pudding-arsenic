...
class App extends React.Component {
  ...
  randomIngredientNumber() {
    ...
    const idNumbers = Math.floor(Math.random() * 5);
  ...
  render() {
    ...
    const showIngredientCall = <ShowIngredients {...product[5]} />;
    ...
    return (
      <div>
        <header />
        {ingredientArray.map((showIngredientCall) => <div>{showIngredientCall}</div>)}
      </div>
    );
  }
}
